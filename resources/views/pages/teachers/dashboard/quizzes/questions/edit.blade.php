...
@section('title')
    {{ trans('Questions_trans.edit_title') }}
@stop
                    ...
                    <div class="card-body">
                        <form action="{{ route('teacher.quizzes.questions.update', [$quiz->id, $question->id]) }}"
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="answers" name="answers" rows="4" class="form-control" required>{{ old('answers', $question->answers) }}</textarea>
                                ...
                                <div class="col-md-4 mb-3">
                                    ...
                                    <select id="score" name="score" class="custom-select" required>
                                        @foreach ([5, 10, 15, 20] as $score)
                                            <option value="{{ $score }}"
                                                {{ (int) old('score', $question->score) === $score ? 'selected' : '' }}>
                                                {{ $score }}
                                            </option>
                                        @endforeach
                                    </select>
