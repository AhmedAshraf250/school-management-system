...
@section('title')
    {{ trans('Quizzes_trans.edit_title') }}
@stop
...
@section('content')
    ...
    {{-- Teacher edit quiz page --}}
    <div class="teacher-view-scope">
        <div class="row">
            <div class="col-12 mb-30">
                <div class="card card-statistics h-100">
                    <div class="card-body">
                        <form action="{{ route('teacher.quizzes.update', $quiz->id) }}" method="POST" autocomplete="off">
                            @csrf
                            @method('PUT')
                                    ...
                                    <select id="section_id" name="section_id" class="custom-select" required>
                                        @foreach ($teacherSections as $section)
                                            <option value="{{ $section->id }}" data-grade-id="{{ $section->grade_id }}"
                                                ...
                                                {{ (int) old('section_id', $quiz->section_id) === (int) $section->id ? 'selected' : '' }}>
                            ...
                            <div class="form-row">
                                <div class="col-md-4 mb-3">
                                    ...
                                    <input id="academic_year" type="text" name="academic_year" class="form-control"
                                        value="{{ old('academic_year', $quiz->academic_year) }}" required>
                                </div>
                            </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const rebuildSubjectOptions = function() {
                ...
                if (hasCurrentSubject) {
                    subjectSelect.value = currentSelectedSubjectId;
                    return;
                }

                subjectSelect.selectedIndex = subjectSelect.options.length > 0 ? 0 : -1;
            };
