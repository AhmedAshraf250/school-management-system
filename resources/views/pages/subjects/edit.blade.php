...
@section('title')
    {{ trans('Subjects_trans.edit_title') }}
@endsection
                    ...
                    <form action="{{ route('subjects.update', $subject->id) }}" method="post" autocomplete="off">
                        ...
                        <div class="form-row">
                            <div class="col">
                                ...
                                <input id="name_ar" type="text" name="name_ar" class="form-control"
                                    value="{{ old('name_ar', $subject->getTranslation('name', 'ar')) }}" required>
                            </div>
                            <div class="col">
                                <label for="name_en">{{ trans('Subjects_trans.name_en') }}</label>
                                <input id="name_en" type="text" name="name_en" class="form-control"
                                    value="{{ old('name_en', $subject->getTranslation('name', 'en')) }}" required>
                            </div>
                        </div>
                        ...
                        <div class="form-row">
                                ...
                                <select class="custom-select" name="grade_id" id="grade_id" required>
                                    @foreach ($grades as $grade)
                                        <option value="{{ $grade->id }}"
                                            @selected(old('grade_id', $subject->grade_id) == $grade->id)>
                                            {{ $grade->Name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group col">
                                ...
                                <select name="classroom_id" id="classroom_id" class="custom-select" required>
                                    @foreach ($classrooms as $classroomId => $classroomName)
                                        <option value="{{ $classroomId }}"
                                            @selected(old('classroom_id', $subject->classroom_id) == $classroomId)>
                                            {{ $classroomName }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select class="custom-select" name="teacher_id" id="teacher_id" required>
                                    @foreach ($teachers as $teacher)
                                        <option value="{{ $teacher->id }}"
                                            @selected(old('teacher_id', $subject->teacher_id) == $teacher->id)>
                                            {{ $teacher->name }}
                                        </option>
                                    @endforeach
                                </select>
